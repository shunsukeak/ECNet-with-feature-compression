...
def parse_args():
    ...
    parser.add_argument(
        "--anno_path", type=Path, default="./data/COCO/annotations/instances_train2017.json",
        help="json filename",
    )
    parser.add_argument(
        "--weights", type=Path, default="./weights/yolov3.weights",
        # "--weights", type=Path, default="./train_output102/yolov3-edge_010000.ckpt",
    ...
    parser.add_argument(
        ...
        help="GPU id to use")
    parser.add_argument(
        "--save_dir", type=Path, default="train_output103",
...
def main():
    ...
    for iter_i in range(start_iter, max_iter + 1):
        ...
        if iter_i % args.save_interval == 0 or iter_i == max_iter:
            ...
            # Loss kiroku (?)
            hor = [item["iter"] for item in history]

            ver_total_train = [item["loss_total"] for item in history]

            fig, ax = plt.subplots()

            ax.plot(hor, ver_total_train, marker="o", color = "blue", linestyle = "--",label="train_loss")
            ax.set_ylabel('loss')
            ax.set_xlabel('iteration')
        
            
            plt.savefig(str(args.save_dir)+'/history2/'+ str(iter_i) +'.jpg')
            plt.clf()
            plt.close()
